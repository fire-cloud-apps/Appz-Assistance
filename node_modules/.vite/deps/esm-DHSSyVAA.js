...
function useMouse(options = { resetOnExit: false }) {
	...
	const setMousePosition = (event) => {
		if (ref.current) {
			...
			setPosition({
				x: Math.max(0, Math.round(event.pageX - rect.left - (window.scrollX || window.scrollX))),
				y: Math.max(0, Math.round(event.pageY - rect.top - (window.scrollY || window.scrollY)))
			});
		} else setPosition({
			x: event.clientX,
			y: event.clientY
...
//# sourceMappingURL=esm-DHSSyVAA.js.map